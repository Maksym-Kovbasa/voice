...
describe('removeProfileFieldValue', () => {
  it('removes normalized recommended links', () => {
    const fields = {
      recommended_links: ['imdb.com/title/tt5580390', 'https://flutter.dev'],
    };

    const next = removeProfileFieldValue(fields, 'recommended_links', 'https://imdb.com/title/tt5580390');
    expect(next.recommended_links).toEqual(['https://flutter.dev']);
